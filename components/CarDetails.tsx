          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white p-6 text-left shadow-xl transition-all flex flex-col gap-5">
                ...
                <div className="relative w-full h-52 bg-cover bg-center rounded-lg">
                  <Image src={generateCarImageUrl(car)} alt={`${car.make} ${car.model}`} fill className="object-cover rounded-lg" />
                </div>
                ...
                <div className="flex gap-3 mt-4">
                  {[29, 33, 13].map((angle) => (
                    <div key={angle} className="flex-1 relative w-full h-24 bg-primary-blue-100 rounded-lg">
                      <Image src={generateCarImageUrl(car, `${angle}`)} alt="car model" fill className="object-cover rounded-lg" />
                    </div>
                  ))}
                </div>
